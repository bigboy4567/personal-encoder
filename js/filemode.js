...
//   déchiffré = filename_len(2, big-endian) | filename (UTF-8) | contenu du fichier
...
export async function encodeFile(filename, fileBytes, passphrase) {
  ...
  const plaintext = concatBytes(filenameLen, filenameBytes, fileBytes);
...
export async function decodeFile(pcmInt16, passphrase) {
  ...
  if (plaintext.length < 2) throw new FileDecodeError("en-tête de nom de fichier manquant");
  const filenameLen = new DataView(plaintext.buffer, plaintext.byteOffset, 2).getUint16(0, false);
  const filename = new TextDecoder().decode(plaintext.slice(2, 2 + filenameLen));
  const fileBytes = plaintext.slice(2 + filenameLen);
